...
use Fxp\Component\Mailer\Loader\TemplateMailLoaderChain;
...
/**
 * Tests for chain template layout loader.
 *
 * @author François Pluchino <francois.pluchino@gmail.com>
 *
 * @internal
 */
final class TemplateMailLoaderChainTest extends TestCase
{
    public function testLoad(): void
    {
        $template = $this->getMockBuilder(TemplateMailInterface::class)->getMock();

        $loader1 = $this->getMockBuilder(TemplateMailLoaderInterface::class)->getMock();
        $loader1->expects($this->once())
            ->method('load')
            ->willThrowException(new UnknownMailException('test', MailTypes::TYPE_ALL))
        ;

        $loader2 = $this->getMockBuilder(TemplateMailLoaderInterface::class)->getMock();
        $loader2->expects($this->once())
            ->method('load')
            ->will($this->returnValue($template))
        ;

        $chainLoader = new TemplateMailLoaderChain([$loader1, $loader2]);
    ...
    public function testLoadUnknownTemplate(): void
    {
        ...
        $loader = new TemplateMailLoaderChain([]);
